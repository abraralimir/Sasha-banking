  ...
  const generatePdfFromHtml = (
    ...
    if (type === 'loan' && 'eligibility' in report) {
      ...
      const trendsHtml = financialReport.trendsAndGraphs ? `
        <h2 style="font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 5px;">${selectedTitles.trendsAndGraphsTitle}</h2>
        <p style="font-size: 11px; line-height: 1.6; white-space: pre-wrap;">${financialReport.trendsAndGraphs}</p>
      ` : '';
      
      const flawsHtml = (financialReport.identifiedFlaws && financialReport.identifiedFlaws.length > 0) ? `
        <h2 style="font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 5px;">${selectedTitles.identifiedFlawsTitle}</h2>
        <ul style="padding-${isRtl ? 'right' : 'left'}: 20px; margin: 0; list-style: none;">
          ${financialReport.identifiedFlaws.map(flaw => `
            <li style="display: flex; align-items: flex-start; margin-bottom: 8px;">
              <span style="height: 8px; width: 8px; margin-top: 5px; margin-${isRtl ? 'left' : 'right'}: 10px; flex-shrink: 0; border-radius: 50%; background-color: #ef4444;"></span>
              <span style="font-size: 11px; line-height: 1.6;">${flaw}</span>
            </li>
          `).join('')}
        </ul>
      ` : '';
      ...
      reportContentHtml = `
        <h2 style="font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 5px;">${selectedTitles.summary}</h2>
        <p style="font-size: 11px; line-height: 1.6; white-space: pre-wrap;">${financialReport.summary}</p>
        ${trendsHtml}
        <h2 style="font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 5px;">${selectedTitles.prediction}</h2>
        <p style="font-size: 11px; line-height: 1.6; white-space: pre-wrap;">${financialReport.prediction}</p>
        <h2 style="font-size: 14px; font-weight: bold; margin-top: 15px; margin-bottom: 5px;">${selectedTitles.creditScoreAssessment}</h2>
        <p style="font-size: 11px; line-height: 1.6; white-space: pre-wrap;">${financialReport.creditScorePrediction}</p>
        ${flawsHtml}
      `;
    }
